...
const Committee = Parse.Object.extend('Committees');
const Notification = Parse.Object.extend('Notification');
const Map = Parse.Object.extend('Maps');
const Gallery = Parse.Object.extend('Gallery');
const Sponsor = Parse.Object.extend('Sponsor');
...
var F8UserType = new GraphQLObjectType({
  ...
  fields: () => ({
    ...
    sponsors: {
      type: new GraphQLList(F8SponsorType),
      resolve: () => new Parse.Query(Sponsor).find(),
    },
    committees: {
      type: new GraphQLList(CommitteeType),
      resolve: () => new Parse.Query(Committee).find(),
    },
    gallery:{
      type: new GraphQLList(GalleryType),
      resolve: () => new Parse.Query(Gallery).find(),
    },
    config: {
      type: F8ConfigType,
...
var GalleryType = new GraphQLObjectType({
  name: 'Gallery',
  description: 'Gallery',
  fields: () => ({
    id: globalIdField('Gallery'),
    title: {
      type: GraphQLString,
      resolve: (page) => page.get('title'),
    },
    image: {
      type: GraphQLString,
      resolve: (page) => page.get('image').url(),
    },
    section: {
      type: GraphQLString,
      resolve: (section) => section.get('section'),
    },
...
var F8SponsorType = new GraphQLObjectType({
  name: 'Sponsor',
...
var CommitteeType = new GraphQLObjectType({
  name: 'Committee',
  fields: () => ({
    id: globalIdField('Committee'),
    name: {
      type: GraphQLString,
      resolve: (speaker) => speaker.get('speakerName'),
    },
    ...
    committee: {
      type: GraphQLString,
      resolve: (speaker) => speaker.get('committee'),
    }
  }),
  interfaces: [nodeInterface],
});
